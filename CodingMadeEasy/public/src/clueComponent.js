...
function doroomactivities(roomnumber) {
	...
	if (currentroom != roomnumber) {
		...
		if (roomentrances > 125) { warning = ('<div style="color: orange;">Jeronimo has grown suspicious of your investigation!</div>'); }
		if (roomentrances > 200) {
			warning = ('<div style="color: red;">Jeronimo is now stalking you!</div>');
			var death = RandomInteger (1,20);
			if ( death == 300 ) {
				lockdoors();
				Crafty.audio.play("strings");
				deadmessage = ('<div style="color: red;">' + character[murderer][0] + ' ' + character[murderer][1] + ' slams the door behind you as you enter. "You were getting just a little too close to the truth Jeronimo doesnt approve of that. Now it`s time for you to suffer the same fate as poor ' + character[victim][0] + '."</div><br/><br/>You are dead. Score: ' + score + '. Click <a href="clue.html">here</a> to play again.');
				warning="";
				dead=true;
			}
		}
...
function processinput() {
	...
	if (commands[0] == "QUESTION") { 
		...
		if (hasmurderweapon == false) { tempmessage = "But you don't have the tools and knowlegde to escape with!"; }
		else {
		...
		if (accuseready == false) { tempmessage = "But you haven't gathered the other people trapped here yet!<br/>"; }
		else { 
			if (commands[2] == null) {
				tempmessage = "You must give the full name of the person you wish to tell.";
			}
			else {
					...
					if (character[i][0].toUpperCase() == commands[1]) {
						if (character[i][1].toUpperCase() == commands[2]) {
							if (murderer!=i) {
								...
								tempmessage = '<font color="Green">"Yes, I know how to get out of here! You`ve got me banged to rights, Senpai. But I think everyone in this room knows it in their hearts LOL Ill open the door now."</font><br/><br/>Score: ' + score + '. Click <a id="dofacebook" href="#" onclick="posttofb();">here</a> to share your score on Facebook, or <a href="sleuth.html">here</a> to play again.';
								say(tempmessage);
								Crafty.pause();
                Crafty.scene('main');
							}
